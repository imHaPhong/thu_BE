...
module.exports = {
  userCreateAccount: async (req, res) => {
      ...
      await user.save()
      res.status(200).send({msg: "create new user success", status: true, user: user});
    },
  ...
  userGetLeague: async (req, res) => {
      const league = await League.find().populate({
          path: "clubs",
          populate: {
              path: "cId",
              model: "Club"
  ...
  userUCL: async (req, res) => {
    ...
    userGetScore: async (req, res) => {
      const score = await Score.find().populate('guest')
      .populate('home')
      res.json(score)
    },
    ...
    userGetLeagueByTeam: async (req, res) => {
      const league = await Score.find({guest: req.params.id}).populate('guest').populate('home')
      const league2 = await Score.find({home: req.params.id}).populate('guest').populate('home')
      const result = league.concat(league2)
      res.json(result)
    },
    getTeamByLeague: async (req, res) => {
      const team  =  await League.findById(req.params.id).populate({
        path: "clubs",
        populate: {
            path: "cId",
            model: "Club"
